...
export interface AnalyzeOverviewInput {
  path?: string;
  title?: string;
  description?: string;
  output_path?: string;
...
export function executeAnalyzeOverview(input: AnalyzeOverviewInput): ToolExecutionResult {
  ...
  const topBranches = Object.entries(
    topology.files.reduce((acc: { [dir: string]: string[] }, file) => {

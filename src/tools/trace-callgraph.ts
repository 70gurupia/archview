...
export interface TraceCallGraphInput {
  path?: string;
  symbol_name: string;
  file_path?: string;
  depth?: number;
...
export function generateCallGraphMermaid(
  symbolName: string,
  targetFile: string | undefined,
  inbound: { callerFile: string; callerSymbol: string; count: number }[],
  outbound: { targetFile: string; calleeSymbol: string; count: number }[],
  direction: string = 'LR'
): string {
  ...
  const inboundByFile: { [file: string]: string[] } = {};
  inbound.forEach(inb => {
    if (!inboundByFile[inb.callerFile]) inboundByFile[inb.callerFile] = [];
  ...
  const outboundByFile: { [file: string]: string[] } = {};
  outbound.forEach(out => {
    if (!outboundByFile[out.targetFile]) outboundByFile[out.targetFile] = [];
...
export function executeTraceCallGraph(input: TraceCallGraphInput): ToolExecutionResult {
  ...
  if (!matchedFile) {
    const foundFile = topology.files.find(f =>
      f.symbols.some(s => s.name === targetSymbol) || f.exports.includes(targetSymbol)
    );
    if (foundFile) matchedFile = foundFile.relativePath;
  }

  // Find Inbound callers: Who calls targetSymbol or matchedFile?
  const inbound: { callerFile: string; callerSymbol: string; count: number }[] = [];
  ...
  const outbound: { targetFile: string; calleeSymbol: string; count: number }[] = [];
  if (matchedFile) {
    topology.crossModuleCalls.forEach(call => {

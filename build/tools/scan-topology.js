...
export function generateTopologyMermaid(topology, viewMode = 'hybrid', direction = 'TD') {
    ...
    if (viewMode === 'layered') {
        ...
        topology.files.forEach(f => {
            if (f.layer === 'controller')
                layerGroups['🎮 Controladores e Rotas'].push(f);
            else if (f.layer === 'service')
                layerGroups['⚙️ Serviços e Regras de Negócio'].push(f);
            else if (f.layer === 'repository' || f.layer === 'model')
                layerGroups['💾 Persistência e Banco de Dados'].push(f);
            else if (f.layer === 'middleware')
                layerGroups['🛡️ Middlewares e Segurança'].push(f);
            else if (f.layer === 'client')
                layerGroups['📡 Clientes e Integrações'].push(f);
            else if (f.layer === 'util' || f.layer === 'config')
                layerGroups['🔧 Utilitários e Configurações'].push(f);
            else
                layerGroups['📁 Outros Componentes'].push(f);
        });
        let groupIdx = 0;
...
export function executeScanTopology(input) {

export function parseLexicalFile(filePath, relativePath, sourceText) {
    ...
    if (language === 'Python') {
        if (imports.some(i => i.source.includes('fastapi')) || routes.some(r => r.handler.includes('fastapi')))
            framework = 'FastAPI';
        else if (imports.some(i => i.source.includes('django')))
            framework = 'Django';
        else if (imports.some(i => i.source.includes('flask')))
            framework = 'Flask';
    }
    else if (language === 'Go') {
        if (imports.some(i => i.source.includes('gin-gonic')))
            framework = 'Gin';
        else if (imports.some(i => i.source.includes('gofiber')))
            framework = 'Fiber';
        else if (imports.some(i => i.source.includes('echo')))
            framework = 'Echo';
    }
    else if (language === 'Java' || language === 'Kotlin') {
        if (imports.some(i => i.source.includes('springframework')))
            framework = 'Spring Boot';
        else if (imports.some(i => i.source.includes('quarkus')))
            framework = 'Quarkus';
    }
...
// 1. Python Parser
function parsePython(lines, imports, exports, symbols, calls, routes) {
...
// 2. Go Parser
function parseGo(lines, imports, exports, symbols, calls, routes) {
    let currentScope = 'global';
...
function parseJavaKotlin(lines, imports, exports, symbols, calls, routes) {
    let currentScope = 'global';
...
// 4. Rust Parser
function parseRust(lines, imports, exports, symbols, calls, routes) {
    let currentScope = 'global';
    lines.forEach((lineText, idx) => {
        const lineNum = idx + 1;
        const trimmed = lineText.trim();
...
// 5. PHP Parser
function parsePHP(lines, imports, exports, symbols, calls, routes) {
    let currentScope = 'global';
    lines.forEach((lineText, idx) => {
        const lineNum = idx + 1;
        const trimmed = lineText.trim();
...
// 6. C# Parser
function parseCSharp(lines, imports, exports, symbols, calls, routes) {
    let currentScope = 'global';
    lines.forEach((lineText, idx) => {
        const lineNum = idx + 1;
        const trimmed = lineText.trim();
...
function parseGeneric(lines, imports, symbols, calls) {
    lines.forEach((lineText, idx) => {
        const lineNum = idx + 1;
        const trimmed = lineText.trim();

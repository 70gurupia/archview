import fs from 'fs';
export function parseTraceToSequence(input) {
    ...
    if (input.trace_data) {
        if (Array.isArray(input.trace_data)) {
            ...
            if (Array.isArray(input.trace_data.spans)) {
                spans = input.trace_data.spans.map((item, idx) => normalizeSpan(item, idx));
            }
            else if (Array.isArray(input.trace_data.events)) {
                spans = input.trace_data.events.map((item, idx) => normalizeSpan(item, idx));
            }
            else {
    ...
    spans.forEach(s => {
        if (s.from)
            participantSet.add(s.from);
        if (s.to)
            participantSet.add(s.to);
    });
...
function normalizeSpan(item, idx) {
